...
router.post('/attendance/grant', async (req, res) => {
  try {
    ...
    if (existingReward) {
      ...
      console.log('[출석체크] 이전 FAILED 레코드 삭제 후 재시도:', existingReward.id);
    ...
    while (retryCount <= MAX_RETRY) {
      ...
      try {
        ...
        console.log('[토스 프로모션 지급 응답]', JSON.stringify(executeResult, null, 2));

        // 응답이 왔지만 FAIL인 경우 처리
        if (executeResult.resultType === 'FAIL') {
          const errorCode = executeResult.error?.errorCode;
          const errorReason = executeResult.error?.reason || TOSS_ERROR_MESSAGES[errorCode] || '알 수 없는 오류';

          console.error(`[토스 프로모션 지급 실패] 에러코드: ${errorCode}, 사유: ${errorReason}`);

          // 4110: 재시도 가능한 에러
          if (RETRYABLE_ERROR_CODES.includes(errorCode) && retryCount < MAX_RETRY) {
            retryCount++;
            console.log(`[토스 프로모션] ${errorCode} 에러, 재시도 ${retryCount}/${MAX_RETRY}`);
            // 잠시 대기 후 재시도 (지수 백오프)
        ...
        console.error('[토스 프로모션 지급 HTTP 에러]', JSON.stringify(tossError.response?.data, null, 2));
        lastError = tossError;

        // HTTP 에러도 재시도 시도
        if (retryCount < MAX_RETRY) {
          retryCount++;
          console.log(`[토스 프로모션] HTTP 에러, 재시도 ${retryCount}/${MAX_RETRY}`);
          await new Promise(resolve => setTimeout(resolve, 1000 * retryCount));
          continue;
        }
        ...
        await prisma.pointReward.create({
          data: {
            ...
            rewardDate: todayKST,
            status: 'FAILED',
            errorCode: tossError.response?.data?.error?.errorCode?.toString() || 'HTTP_ERROR',
            errorMessage: tossError.response?.data?.error?.reason || tossError.message
          }
        });
